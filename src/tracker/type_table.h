...
class TypeTable {
	...
	int convert_hex_to_color(const std::wstring& s) {
		auto parse_rgb = [](const std::wstring& hex) {
			try {
				const int rgb = std::stoi(hex, nullptr, 16);
		...
		if (s.size() == 6) return parse_rgb(s);
		return -1;
	}

public:

	TypeTable() noexcept = default;
